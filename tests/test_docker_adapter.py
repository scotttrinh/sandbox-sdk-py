...
async def test_async_docker_open_context_manager_roundtrip() -> None:
    """Async connect -> async fs.open("w") -> write -> async fs.open("r") -> read back."""
    ...
    async with connect(image="alpine:latest") as sbx:
        ...
        async with sbx.fs.open(test_path, "w") as f:
            assert f.writable()
            await f.write(test_content)

        # Standard open() context manager in read mode
        async with sbx.fs.open(test_path, "r") as f:
            assert f.readable()
            result = await f.read()
...
def test_sync_docker_open_context_manager_roundtrip() -> None:
    """Sync connect_sync -> fs.open("wb") -> write -> fs.open("rb") -> read back."""
    ...
    with connect_sync(image="alpine:latest") as sbx:
        with sbx.fs.open(test_path, "wb") as f:
            assert f.writable()
            f.write(test_content)

        with sbx.fs.open(test_path, "rb") as f:
            assert f.readable()
            result = f.read()
            assert result == test_content
...
async def test_read_non_existent_file_raises_not_found() -> None:
    ...
    async with connect(image="alpine:latest") as sbx:
        with pytest.raises(SandboxPathNotFoundError):
            async with sbx.fs.open("/non_existent/missing.txt", "r"):
                pass
